  ...
  Future<void> start() async {
    ...
    _server!.listen((HttpRequest request) async {
      ...
      if (WebSocketTransformer.isUpgradeRequest(request)) {
        ...
        if (packetLossRate > 0 &&
            DateTime.now().millisecond % 100 < (packetLossRate * 100)) {
...
void main() {
  group('Weak Network Tests', () {
    ...
    setUp(() async {
      ...
      proxy = NetworkConditionProxy(
          targetPort: 0,
          proxyPort: port); // targetPort unused as it acts as server
      await proxy.start();
    });
    ...
    test('should eventually connect with 50% handshake failure rate', () async {
      ...
      if (client.state != SocketState.connected) {
        print(
            'Initial connection failed (expected), waiting for auto-reconnect...');
